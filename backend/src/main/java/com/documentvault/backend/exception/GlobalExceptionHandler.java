...
public class GlobalExceptionHandler{
    ...
    public ResponseEntity<ErrorResponse> handleValidationException(MethodArgumentNotValidException ex){
        List<String> errors=ex.getBindingResult()
                ...
                .toList();
        ErrorResponse response=new ErrorResponse(LocalDateTime.now(),HttpStatus.BAD_REQUEST.value(),String.join(", ",errors));
    ...
    public ResponseEntity<ErrorResponse> handleException(Exception ex){
        ErrorResponse response=new ErrorResponse(LocalDateTime.now(),HttpStatus.INTERNAL_SERVER_ERROR.value(), "internal server error.");
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
    }
}